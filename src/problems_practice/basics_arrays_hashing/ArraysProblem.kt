...
class MyHashMap() {
    ...
    private val totalCapacity = 1000001
    private val storageArray = IntArray(totalCapacity){ 0 }

    fun getHashKey(key: Int): Int{
        return key%totalCapacity
    }

    fun put(key: Int, value: Int) {
        val hKey = getHashKey(key)
        storageArray[hKey] = value
    }

    fun get(key: Int): Int {
        val hKey = getHashKey(key)
        if(storageArray[key] == 0){
            return -1
        }
